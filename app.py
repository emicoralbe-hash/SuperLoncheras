...
async def read_item(request: Request, q: str = None):
    ...
    return templates.TemplateResponse(request=request, name="index.html", context={"recetas": recetas, "query": q, "user": request.session.get("user")})

@app.get("/planner", response_class=HTMLResponse)
async def get_planner(request: Request):
    db = load_db()
    return templates.TemplateResponse(request=request, name="planner.html", context={"recetas": db.get("recetas", []), "dias": range(1, 31), "user": request.session.get("user")})
...
async def get_batch_cooking(request: Request):
    ...
    return templates.TemplateResponse(request=request, name="batch_cooking.html", context={"recetas": frozen, "user": request.session.get("user")})

@app.get("/marketplace", response_class=HTMLResponse)
async def get_marketplace(request: Request):
    db = load_db()
    return templates.TemplateResponse(request=request, name="marketplace.html", context={"productos": db.get("productos", []), "user": request.session.get("user")})

@app.get("/recipe/{recipe_id}", response_class=HTMLResponse)
async def get_recipe(request: Request, recipe_id: int):
    db = load_db()
    receta = next((r for r in db["recetas"] if r["id"] == recipe_id), None)
    return templates.TemplateResponse(request=request, name="recipe_detail.html", context={"receta": receta, "user": request.session.get("user")})
...
async def admin_panel(request: Request):
    ...
    db = load_db()
    return templates.TemplateResponse(request=request, name="admin.html", context={"db": db, "user": user})
...
async def add_recipe(request: Request, nombre: str = Form(...), tag: str = Form(...), img: str = Form(...), steps: str = Form(...), video: str = Form(...)):
    ...
    new_recipe = {"id": new_id, "nombre": nombre, "tag": tag, "img": img, "ingredientes": [], "alergias": [], "pasos": steps.split("\n"), "video_url": video}
